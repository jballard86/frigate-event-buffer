...
- **Method:** POST  
- **URL:** From config (e.g. `GEMINI_PROXY_URL`), e.g. `http://192.168.21.189:5050/v1/chat/completions` (exact path to be confirmed for your proxy).  
- **Headers:**  
  - `Content-Type: application/json`  
  - `Authorization: Bearer <GEMINI_PROXY_API_KEY>` (if proxy uses bearer token; confirm separately).  
