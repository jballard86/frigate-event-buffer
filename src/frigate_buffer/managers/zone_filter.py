...
class SmartZoneFilter:
    ...
    def should_start_event(
        ...
        When tracked_zones is set, we start if the object is in a tracked zone per
        either entered_zones or current_zones, so we react on the first message where
        the object is currently in zone even if entered_zones is empty (avoids delayed
        first notification when Frigate populates zone only in later messages).
        """
