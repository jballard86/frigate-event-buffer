...
class EventLifecycleService:
    ...
    def finalize_consolidated_event(self, ce_id: str):
        ...
        if first_clip_path is None and primary_cam and ce.primary_event_id:
            ...
            ok = self.download_service.download_and_transcode_clip(ce.primary_event_id, primary_folder, primary_cam)
            if ok:
        ...
        if first_clip_path:
            ...
            try:
                ...
                compile_ce_video(ce.folder_path, float(end_ts - start_ts), self.config, primary_cam)
            except Exception as e:
                logger.error(f"Error executing compilation hook for {ce_id}: {e}")
