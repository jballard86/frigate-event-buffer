"""EventQueryService: list caches, disk layout, and timeline-derived event dicts."""
...
from frigate_buffer.services.query.fs_storage import parse_event_folder
from frigate_buffer.services.query.parsing import (
    event_ended_in_timeline,
    extract_cameras_zones_from_timeline,
    extract_end_timestamp_from_timeline,
    extract_genai_entries,
    parse_summary,
)
from frigate_buffer.services.query.query_cache import (
    FolderEventParseCache,
    TtlListCache,
)
...
class EventQueryService:
    ...
    def __init__(
        ...
        self._list_cache = TtlListCache(cache_ttl)
        self._folder_cache = FolderEventParseCache(event_cache_max, parse_event_folder)
    ...
    def _get_event_cached(self, folder_path: str, mtime: float) -> dict[str, Any]:
        ...
        return self._folder_cache.get(folder_path, mtime)
    ...
    def _get_consolidated_events(self) -> list[dict[str, Any]]:
        ...
        for entry in entries:
            ...
            genai_entries = extract_genai_entries(timeline)
            ...
            event_ended = event_ended_in_timeline(timeline)
            try:
                ts_float = float(ts)
            ...
            cameras_with_zones = extract_cameras_zones_from_timeline(timeline)
            if not cameras_with_zones and cameras:
                cameras_with_zones = [{"camera": c, "zones": []} for c in cameras]

            end_ts = extract_end_timestamp_from_timeline(timeline) or metadata.get(
                "end_time"
            )
            event_dict = {
                "event_id": ce_id,
    ...
    def get_events(self, camera_name: str) -> list[dict[str, Any]]:
        ...
        self._list_cache.set(cache_key, events)
        return events
    ...
    def get_saved_events(self, camera: str | None = None) -> list[dict[str, Any]]:
        ...
        if not os.path.isdir(saved_path):
            self._list_cache.set(cache_key, all_events)
            return all_events
    ...
    def get_test_events(self) -> list[dict[str, Any]]:
        ...
        if not os.path.isdir(events_dir):
            self._list_cache.set(cache_key, events_list)
            return events_list
    ...
    def get_unread_count(self) -> int:
        ...
        Uses os.scandir only; does not run parse_event_folder. Result is cached
        with the same TTL as list caches to avoid disk thrashing when the
        mobile app polls frequently.
        """
        cached = self._list_cache.get("unread_count")
        if cached is not None:
            return cached
        ...
        self._list_cache.set("unread_count", count)
        return count
