...
def _remove_other_mp4_in_folder(folder_path: str, keep_path: str) -> None:
    """Delete any .mp4 in folder_path that is not keep_path (orphan GC after successful write)."""
...
class DownloadService:
    ...
    def export_and_download_clip(
        ...
        """
        Request clip export from Frigate's Export API, then download directly to dynamic path (no transcode).

        Uses buffer-assigned event time range. Saves as {camera}-{unix_timestamp}.mp4 and removes
        any other .mp4 in the folder (orphan GC). Falls back to events API if export fails.

        Returns dict: success (bool), frigate_response (dict), clip_path (str | None), optionally fallback (str).
        """
        ...
        try:
            ...
            if resp.ok and frigate_response and isinstance(frigate_response, dict) and frigate_response.get("success") is False:
                logger.warning("Export failed for %s. Status: %s. Response: %s", event_id, resp.status_code, resp.text or "(empty)")
            ...
            if not export_filename:
                ...
                fallback_result = self._download_clip_events_api_to_path(event_id, folder_path, camera)
                return {
            ...
            logger.info("Downloaded export clip for %s (%s bytes) to %s", event_id, bytes_downloaded, final_path)
            return {"success": True, "frigate_response": frigate_response, "clip_path": final_path}

        except requests.exceptions.RequestException as e:
            if getattr(e, "response", None) is not None and e.response.text:
                logger.warning("Export API request failed for %s: response.text=%s", event_id, e.response.text)
            else:
                logger.warning("Export API failed for %s: %s, falling back to events API", event_id, e)
            fallback_result = self._download_clip_events_api_to_path(event_id, folder_path, camera)
            return {
                "success": fallback_result.get("success", False),
            ...
            if os.path.exists(final_path):
                try:
                    ...
                    pass
            return {"success": False, "frigate_response": frigate_response or {"error": str(e)}, "clip_path": None}

    def _download_clip_events_api_to_path(
        self, event_id: str, folder_path: str, camera: str
    ) -> dict:
        """Download clip from Frigate events API to dynamic path; run orphan GC. Returns success, clip_path."""
        final_path = os.path.join(folder_path, _dynamic_clip_basename(camera))
                        ...
                        with open(final_path, "wb") as f:
                            for chunk in response.iter_content(chunk_size=8192):
                                if chunk:
                                    ...
                                    bytes_downloaded += len(chunk)
                        _remove_other_mp4_in_folder(folder_path, final_path)
                        logger.debug("Downloaded clip for %s (%s bytes) to %s", event_id, bytes_downloaded, final_path)
                        return {"success": True, "clip_path": final_path}
                except requests.exceptions.HTTPError as e:
                    if e.response is not None:
                        if e.response.status_code == 404:
                            logger.warning("No recording available for event %s", event_id)
                            return {"success": False, "clip_path": None}
                        if e.response.status_code == 400 and attempt < 3:
                            logger.warning("Clip not ready for %s (HTTP 400), retrying in 5s (%s/3)", event_id, attempt)
                            time.sleep(5)
                            continue
                    raise
            return {"success": False, "clip_path": None}
    ...
    def download_clip_to_temp(self, event_id: str, folder_path: str, camera: str) -> dict:
        """
        Download clip from Frigate events API to dynamic path (no transcode).
        Returns dict with success (bool), clip_path (str | None). Runs orphan GC after write.
        """
        return self._download_clip_events_api_to_path(event_id, folder_path, camera)

    def download_and_transcode_clip(self, event_id: str, folder_path: str, camera: str) -> bool:
        """Download clip from Frigate events API to dynamic path (fallback when Export API fails). No transcode."""
