...
The base image (e.g. `jrottenberg/ffmpeg:7.0-nvidia2204`) uses standard Linux paths: FFmpeg and ffprobe are in `/usr/local/bin`, and shared libs in `/usr/local/lib`. The Dockerfile copies from those locations into the final image (not from a custom `/opt/ffmpeg` layout). The **app image is based on Ubuntu 22.04** so it matches the FFmpeg donor; this avoids distro/ABI mismatch (e.g. with Debian) that can cause FFmpeg to silently omit NVENC when NVIDIA driver libs are injected at runtime.
