...
| `src/frigate_buffer/services/video_compilation.py` | Video compilation service. Generates a stitched, cropped, 20fps summary video for a CE lifecycle; uses software decode so filter_complex (crop, format, concat) runs on CPU with consistent pixel formats, then GPU encode (h264_nvenc). Filter chain includes `format=yuv420p` to standardize pixel formats before concat when mixing inputs with different color ranges (e.g., yuv420p vs yuvj420p). | Used by lifecycle, orchestrator. |
