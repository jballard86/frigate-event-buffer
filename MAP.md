...
| `src/frigate_buffer/services/video.py` | **VideoService:** **NeLux** NVDEC decode for detection sidecars (one `VideoReader` per clip with **num_threads=1**; batched `get_batch` with **BATCH_SIZE=16**, float32/255 normalization for YOLO; `del` + `torch.cuda.empty_cache()` after each batch). **GPU_LOCK** (threading.Lock) serializes all NeLux VideoReader and get_batch access app-wide; video_compilation and multi_clip_extractor import and use it so NVDEC is never used concurrently. Uses **sanitize_for_nelux** before opening VideoReader in `generate_detection_sidecar` (sanitizer not under lock). No ffmpegcv/CPU decode; NeLux open or decoder-not-ready failures log **NVDEC_INIT_FAILURE_PREFIX** (constants) at ERROR before return. If reader is missing `_decoder`, monkey-patch `reader._decoder = reader` so existing code works. `_nelux_frame_count(reader, fps, duration)` gets frame count from reader or ffprobe fallback when reader's `len()` fails. `get_detection_model_path(config)` (model under `STORAGE_PATH/yolo_models/`), `generate_detection_sidecar`, `generate_detection_sidecars_for_cameras` (shared YOLO + lock), `run_detection_on_image` (numpy or tensor, normalized for YOLO; quick-title), `generate_gif_from_clip` (subprocess FFmpeg). Single `_get_video_metadata` ffprobe per clip. App-level sidecar lock injected by orchestrator. **Crash debugging:** `generate_detection_sidecar` and `_run_detection_on_batch` use aggressive INFO logging with `_flush_logger()` at sanitizer, VideoReader, frame count, each batch request/return, tensor normalization, and YOLO/xyxy conversion so the last log line identifies the failing step when the C++ backend segfaults. | Used by lifecycle, ai_analyzer, event_test. |
...
| `src/frigate_buffer/services/video_sanitizer.py` | **GPU video sanitizer for NeLux:** context manager `sanitize_for_nelux(clip_path)` re-encodes clips with FFmpeg (CUDA decode + scale_cuda for width cap, h264_nvenc profile high, **-g 1** All-I-frame) into a temp file (mkstemp in `/dev/shm` when available); 100% GPU/VRAM; All-I-frame so NeLux NVDEC does not hit "Slice overlaps with next" on strided/random access. Yields temp path on success or original path on failure (log stderr); strict cleanup in `finally`. Output is H.264 only (NeLux HEVC bug); ultra-wide scaled in VRAM; -b:v 10M for All-I bandwidth. Used so corrupted input from the NVR does not crash NeLux. | Used by video, video_compilation, multi_clip_extractor. |
...
| `src/frigate_buffer/services/video_compilation.py` | Video compilation service. Generates a stitched, cropped, 20fps summary video for a CE lifecycle; uses the **same timeline config** as the frame timeline (MAX_MULTI_CAM_FRAMES_SEC, MAX_MULTI_CAM_FRAMES_MIN) and **one slice per assignment** so camera swapping matches the AI prompt timeline. Crop follows the tracked object with **smooth panning** (tensor crop with 0-based `t/duration` interpolation; optional EMA on crop center). On the **last slice of a camera run** (slice before a camera switch), crop is **held** (`crop_end = crop_start`). **NeLux** (vendored wheel): NVDEC decode per slice with **num_threads=1**; each slice opened via **sanitize_for_nelux** before VideoReader; uses **GPU_LOCK** from video around VideoReader + frame count and each get_batch; tensor crop (smooth pan). **Encode:** FFmpeg subprocess **h264_nvenc only** (GPU; no CPU fallback); **-thread_queue_size 512** before `-i pipe:0`; **-preset p1 -tune hq -rc vbr -cq 24**; rawvideo stdin; on failure, descriptive error logging (command, returncode, stderr). NVDEC init failures log **NVDEC_INIT_FAILURE_PREFIX** at ERROR. Reader monkey-patch: if `_decoder` missing, set `reader._decoder = reader`. 20fps, no audio, output MP4. **Import order:** `import torch` before `from nelux import VideoReader` (required by NeLux). | Used by lifecycle, orchestrator. |
