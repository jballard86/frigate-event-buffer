...
| `src/frigate_buffer/event_test/event_test_orchestrator.py` | Allocates `events/testN`, copies source, delegates to VideoService (sidecars) and same multi-clip extractor; no YOLO/lock in this module. Does not change logger level; attaches **StreamCaptureHandler** during the run so the test-run page streams all non-MQTT Python log output. | Used only for TEST button; production logic in ai_analyzer and multi_clip_extractor. |
