...
| `src/frigate_buffer/config.py` | Load/validate YAML + env via Voluptuous `CONFIG_SCHEMA`; flat keys (e.g. `MQTT_BROKER`, `GEMINI_PROXY_URL`, `MAX_EVENT_LENGTH_SECONDS`). Optional **notifications.mobile_app** (enabled, credentials_path, project_id) → `NOTIFICATIONS_MOBILE_APP_ENABLED`, `MOBILE_APP_GOOGLE_APPLICATION_CREDENTIALS`, `MOBILE_APP_FIREBASE_PROJECT_ID`; env **GOOGLE_APPLICATION_CREDENTIALS** overrides credentials path, **GOOGLE_CLOUD_PROJECT** overrides project ID for FCM. Frame limits for AI: `multi_cam.max_multi_cam_frames_*` only. Invalid config exits 1. | Used by `main.py`. |
