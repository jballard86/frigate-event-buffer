...
| `src/frigate_buffer/services/video_compilation.py` | Video compilation service. Generates a stitched, cropped, 20fps summary video for a CE lifecycle; uses the **same timeline config** as the frame timeline (MAX_MULTI_CAM_FRAMES_SEC, MAX_MULTI_CAM_FRAMES_MIN) and **one slice per assignment** so camera swapping matches the AI prompt timeline. **compile_ce_video** may extend the passed global_end from sidecar content (max last timestamp_sec across cameras) when the clip on disk is longer than the requested export window, so the summary covers the full content; logs INFO when extending. Slices are **trimmed to an action window** (first/last detection across all cameras ± 3s pre/post roll) to avoid empty footage at the end. Slices carry **native_width** and **native_height** (from sidecar) for decoder-space scaling. **Dynamic zoom:** when **TRACKING_TARGET_FRAME_PERCENT** > 0 and detections exist, crop size is derived from content (bbox + ZOOM_CONTENT_PADDING) vs target percent; **smooth_zoom_ema** (COMPILATION_ZOOM_SMOOTH_EMA_ALPHA) smooths zoom over slices; center reclamped so crop stays in bounds. Crop follows the tracked object with **smooth panning** (t_progress 0–1, center and crop size interpolated in decoder space); when the nearest sidecar entry has no detections (e.g. person left frame), **hold last-known crop** by using the nearest entry with detections within ~5s instead of panning to center. All crop/resize in the frame loop via **crop_utils.crop_around_center_to_size** (variable crop_w/crop_h, fixed output size; bicubic). On the **last slice of a camera run** (slice before a camera switch), crop is **held** (`crop_end = crop_start`). **PyNvVideoCodec decode** per slice via **gpu_decoder.create_decoder(clip_path, gpu_id)** under **GPU_LOCK**; PTS-based frame selection (**get_index_from_time_in_seconds(t)**); **_run_pynv_compilation** decodes in chunks of **BATCH_SIZE** (same VRAM pattern as video.py): **ctx.get_frames(chunk_indices)** per chunk, then **del batch** and **torch.cuda.empty_cache()** after each chunk; if a chunk returns fewer frames than requested, repeat last frame of chunk (safe_j) to keep FFmpeg sync; **ctx.get_frames(indices)** → crop_around_center_to_size(frame, cx, cy, crop_w, crop_h, target_w, target_h) → HWC uint8 RGB **streamed** to FFmpeg stdin (no in-memory frame list). **generate_compilation_video** loads detection sidecars (or accepts optional pre-parsed sidecars from compile_ce_video to avoid redundant I/O), sets crop_start/crop_end and native dimensions per slice; **ERROR** log once per camera on fallback (no sidecar entries or no detections at slice start/end; crop from center or nearby detection within 5s). When decoder returns same frame index for all frames (static frame) or fewer frames than requested (e.g. de-dup): log details at **DEBUG**, repeat last frame for remainder (safe index avoids IndexError), and log **INFO** once per camera per event: "Possible stutter or missing frames from {camera}, check original file for confirmation" with path=clip_path. On decoder failure for a slice: log **NVDEC_INIT_FAILURE_PREFIX** at ERROR, skip slice, continue. When **get_frames** returns 0 frames (post-decode): log ERROR with clear post-decode message (not NVDEC prefix), skip chunk (causes slight jump in video). PyNvVideoCodec "Duplicates are not supported" warning is re-logged at DEBUG in gpu_decoder and suppressed at default level. **Encode:** FFmpeg subprocess **h264_nvenc only** (GPU; no CPU fallback); **stderr routed to event-folder file `ffmpeg_compile.log`**; **-thread_queue_size 512** before `-i pipe:0`; **-preset p1 -tune hq -rc vbr -cq 24**; rawvideo stdin **-pix_fmt yuv420p**; temp output **_temp.mp4**; on non-zero exit, RuntimeError advises checking **ffmpeg_compile.log**. 20fps, no audio, output MP4. | Used by lifecycle, orchestrator. |
...
| `src/frigate_buffer/web/routes/` | **Blueprints:** `pages` (player, stats-page, daily-review, test-multi-cam), `proxy_routes` (snapshot, latest.jpg), `daily_review` (api/daily-review/*), **api** (cameras, events with **filter=saved** / **filter=test_events**, **POST /keep/<path:event_path>**, delete, **viewed/<path:event_path>**, **events/<path:event_path>/timeline**, files, stats, status; uses **orchestrator.query_service**), `test_routes` (api/test-multi-cam/*; **prepare** GET?subdir= copies source to events/testN, returns test_run_id; **stream** accepts **subdir** (full pipeline) or **test_run** (post-copy only on test folder); **video-request** GET?test_run= deletes clips in test folder, requests new clips via Frigate Export API per camera, streams SSE with request/status/completion per camera; **POST send** for "Send prompt to AI": on success appends test_ai_prompt timeline entry, writes CE summary/metadata, evicts test_events cache). Each module exposes create_bp(orchestrator); routes close over orchestrator. | Registered by server.create_app. |
| `src/frigate_buffer/web/templates/*.html` | Jinja2 templates for player, stats, daily report, timeline, test run. **Test run:** On load with subdir, page calls prepare (copy only); shows "Test run: testN" and two buttons—**Start the test** (stream?test_run=, post-copy pipeline) and **Video request** (video-request?test_run=, Frigate Export API into test folder); same log area for both. **Player:** Filter dropdown includes Unreviewed, All Events, Reviewed, **Saved**, and **Test events** (Test events option shown only when current event is consolidated). Event Details (date, start, end, length) for test events use folder mtime and timeline/metadata end_time. **Timeline** template shows entries including **test_ai_prompt** (Send prompt to AI for test runs). Actions dropdown (Download, **Keep**, View Timeline, TEST, Delete); Keep hidden when event is already saved. Clip selector (multi-cam) in nav bar left of Actions; no "Camera:" label. **Keep** moves event to `saved/` (excluded from retention); TEST works from saved events (subdir `saved/events/ce_id`). Event paths use `event_path` for saved (`saved/camera/subdir`) for delete, viewed, timeline. AI Analysis tile: first line = event title or "future screenshot analysis"; then scene. | Rendered by Flask. |
