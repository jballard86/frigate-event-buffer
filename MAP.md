...
| `src/frigate_buffer/logging_utils.py` | `setup_logging()`, `ErrorBuffer` for stats dashboard. **`set_suppress_review_debug_logs`** / **`should_suppress_review_debug_logs`**: thread-safe flag so TEST button stream suppresses three MQTT review DEBUG logs (Processing review, Review for … N/A, Skipping finalization). | Called from main; ErrorBuffer used by web/server and orchestrator; suppress flag set by test_routes, read by mqtt_handler. |
...
| `src/frigate_buffer/services/query.py` | **EventQueryService:** read event data from filesystem with TTL and per-folder caching; `resolve_clip_in_folder`; list events, timeline merge. **`get_saved_events(camera=None)`** lists events under `saved/` (same dict shape, `saved: True`, file URLs under `/files/saved/...`). **`get_test_events()`** lists only test run events (`events/test1`, `events/test2`, ...). For consolidated events, when `{ce_id}_summary.mp4` exists in the CE root, adds a "Summary video" entry to `hosted_clips` and sets `hosted_clip` to that URL. Excludes non-camera dirs (`NON_CAMERA_DIRS`: ultralytics, yolo_models, daily_reports, daily_reviews, **saved**) from `get_cameras()` and `get_all_events()`. | Flask server (events, stats, player). |
