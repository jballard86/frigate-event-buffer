...
- **Multi-camera CE**: When CE has 2+ cameras, lifecycle for each camera: **download** (`export_and_download_clip`) to a dynamic path `{camera}-{unix_timestamp}.mp4`, then **generate_detection_sidecar** (decode-only pass: ffmpegcv read, YOLO every `detection_frame_interval` frames, write `detection.json`). No transcode; clips are stored as-is. When all cameras have clip + sidecar, lifecycle calls `on_ce_ready_for_analysis` once → `analyze_multi_clip_ce` uses **multi_clip_extractor** with **primary_camera** for first-camera bias, hysteresis, and center-point cropping. **Multi-clip extractor** requires a sidecar for every camera; if any camera lacks `detection.json`, it returns [] (no HOG fallback). When every camera has a sidecar, detection-aligned sampling builds sample times from timestamps where person area &gt; 0; extraction uses sequential read only (no seek). Open decode: try NVDEC then CPU; `decode_second_camera_cpu_only` (default True) uses CPU for 2nd+ cameras to avoid NVDEC contention. Reader death drops that camera only. `_handle_ce_analysis_result` writes summary/metadata to CE root, notifies HA. Writes one `ai_frame_analysis/` + manifest at CE root (color-only frames).
- **Video pipeline**: **No encoding.** Clips are downloaded from Frigate and stored with dynamic names (`{camera}-{unix_timestamp}.mp4`). Decode uses **ffmpegcv** `VideoCaptureNV` (NVDEC) in `ai_analyzer.py`, `multi_cam_recap.py`, `video.py`, and `multi_clip_extractor.py`. ffmpegcv readers expose `.fps`, `__len__`/`.count`—**not** `.get(CAP_PROP_*)`. **VideoService** provides `generate_detection_sidecar(clip_path, sidecar_path, config)` (decode-only read, YOLO every N frames, write `detection.json`) and `generate_gif_from_clip`. At startup, **log_gpu_status** runs nvidia-smi (decode diagnostics); **ensure_detection_model_ready** checks whether the detection model is downloaded. **Docker**: Build from repo root: `docker build -t frigate-buffer:latest .`. FFmpeg from multi-stage build provides NVDEC (decode). Run with NVIDIA Container Toolkit and `NVIDIA_DRIVER_CAPABILITIES=compute,video,utility`. **Install and run:** [INSTALL.md](INSTALL.md).
...
| `src/frigate_buffer/services/ai_analyzer.py` | Gemini proxy integration: motion-aware frame extraction, optional center/smart crop from FrameMetadata, system prompt from file; sends frames to OpenAI-compatible proxy; returns analysis dict; writes `analysis_result.json` (and optional ai_frame_analysis); does not publish to MQTT. For multi-cam CE: `analyze_multi_clip_ce` uses `multi_clip_extractor` (target-centric: requires detection sidecars; no HOG fallback). |
