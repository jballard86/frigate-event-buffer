...
| `src/frigate_buffer/services/video_sanitizer.py` | **GPU video sanitizer for NeLux:** context manager `sanitize_for_nelux(clip_path)` re-encodes clips with FFmpeg (CUDA decode + scale_cuda for width cap, h264_nvenc profile high, bf=0) into a temp file (mkstemp in `/dev/shm` when available); 100% GPU/VRAM; I/P-only to avoid NVDEC buffer assertion. Yields temp path on success or original path on failure (log stderr); strict cleanup in `finally`. Output is H.264 only (NeLux HEVC bug); ultra-wide scaled in VRAM. Used so corrupted input from the NVR does not crash NeLux. | Used by video, video_compilation, multi_clip_extractor. |
