...
| `src/frigate_buffer/web/server.py` | Flask app factory `create_app(orchestrator)`. Routes: `/player`, `/stats-page`, `/daily-review`, `/api/events`, `/api/events/.../snapshot.jpg`, `/api/files`, `/api/daily-review`, `/api/stats`, `/status`; `/events/<camera>/<subdir>/timeline` (timeline page), `/events/<camera>/<subdir>/timeline/download` (merged timeline JSON as attachment; same data as timeline page so download works even when only `notification_timeline_append.jsonl` exists). Uses EventQueryService and `read_timeline_merged`; path safety via file_manager. Player preserves AI Analysis block expand state across auto-refresh. |
