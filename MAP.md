...
| `src/frigate_buffer/services/video_compilation.py` | Video compilation service. Generates a stitched, cropped, 20fps summary video for a CE lifecycle; uses the **same timeline config** as the frame timeline (MAX_MULTI_CAM_FRAMES_SEC, MAX_MULTI_CAM_FRAMES_MIN) and **one slice per assignment** so camera swapping matches the AI prompt timeline. Crop follows the tracked object with **smooth panning** (FFmpeg crop expressions with 0-based `t/duration` interpolation; optional EMA on crop center). On the **last slice of a camera run** (slice before a camera switch), crop is **held** (`crop_end = crop_start`) so there is no pan toward the switch-time position at the cut. **One `-i` input per slice** (no deduplication by camera) to avoid "Output pad already connected" crashes. Software decode, then GPU encode (h264_nvenc); filter chain includes `format=yuv420p` for concat. | Used by lifecycle, orchestrator. |
