...
- **Max event length**: When an event ends, if duration ≥ `max_event_length_seconds` (config, default 120), lifecycle takes the **canceled** path: no clip export or download, no AI analysis or decoding; writes canceled summary, sends notification "Event canceled see event viewer for details", renames event (or CE) folder to append `-canceled`, then returns. Event remains in event view for debug. Orchestrator also checks duration in `_on_clip_ready_for_analysis` and skips `analyze_clip` if over max (defense in depth). For CE, if any constituent event has duration ≥ max, the whole CE is canceled.
...
- **Multi-camera CE**: When CE has 2+ cameras, lifecycle **downloads** each clip (`export_and_download_clip`) to a dynamic path `{camera}-{5_random_digits}.mp4`, then calls **VideoService.generate_detection_sidecars_for_cameras** (parallel, one shared YOLO + lock) (decode-only pass: ffmpegcv read, optionally resize at decode for GPU; YOLO every `detection_frame_interval` frames; bbox scaled to native resolution; write `detection.json` with `native_width`/`native_height` and `entries`). No transcode; clips are stored as-is. When all cameras have clip + sidecar, lifecycle calls `on_ce_ready_for_analysis` once → `analyze_multi_clip_ce` uses **multi_clip_extractor** with **primary_camera** for first-camera bias, hysteresis, and center-point cropping. **Trust-the-EMA pipeline** (default when `camera_timeline_use_ema_pipeline` true): Phase 1 in [timeline_ema.py](src/frigate_buffer/services/timeline_ema.py) builds a dense sample grid, normalizes area by native resolution when cameras differ, runs EMA per camera, assigns segment with hysteresis (switch only when new camera exceeds current by margin), then merges short segments (into previous; first segment rolls forward into next). [multi_clip_extractor](src/frigate_buffer/services/multi_clip_extractor.py) decodes the finalized (T, camera) list and keeps all frames (tags person_area from sidecar). Config: `camera_timeline_use_ema_pipeline`, `camera_timeline_analysis_multiplier`, `camera_timeline_ema_alpha`, `camera_timeline_primary_bias_multiplier`, `camera_switch_min_segment_frames`, `camera_switch_hysteresis_margin`, `camera_timeline_final_yolo_drop_no_person`. Legacy keys remain for backward compat when pipeline is disabled. **Multi-clip extractor** requires a sidecar for every camera; if any camera lacks `detection.json`, it returns [] (no HOG fallback). When person area ≥ `tracking_target_frame_percent` of reference area (min of target crop area and frame area), uses full-frame resize with letterbox and sets `is_full_frame_resize` in frame metadata. Each extracted frame’s metadata includes `person_area` (px²) for that frame. **person_area_debug** (multi_cam, default false): when true, draws that value at bottom-right on each multi-cam frame (same style as timestamp overlay) for tuning `person_area_switch_threshold`. Returns `list[ExtractedFrame]` (frame, timestamp_sec, camera, metadata). Extraction uses sequential read only (no seek). Decode: NVDEC for all cameras with CPU only as fallback when NVDEC fails; `decode_second_camera_cpu_only` (default false) is a legacy option to force 2nd+ cameras to CPU if needed. Reader death drops that camera only. `_handle_ce_analysis_result` writes summary/metadata to CE root, notifies HA. Writes one `ai_frame_analysis/` + manifest at CE root (color-only frames).
- **Video pipeline**: **No encoding.** Clips are downloaded from Frigate and stored with dynamic names (`{camera}-{5_digits}.mp4`, e.g. `doorbell-82749.mp4`). Decode uses **ffmpegcv** `VideoCaptureNV` (NVDEC) in `ai_analyzer.py`, `multi_cam_recap.py`, `video.py`, and `multi_clip_extractor.py`. ffmpegcv readers expose `.fps`, `__len__`/`.count`—**not** `.get(CAP_PROP_*)`. **VideoService** provides `generate_detection_sidecar(clip_path, sidecar_path, config)` (decode-only read, optional resize-at-decode; YOLO every N frames; bbox in native coords; write `detection.json`) and `generate_detection_sidecars_for_cameras(tasks, config)` (parallel sidecar for multiple cameras with one shared YOLO + lock; used by lifecycle and event_test). The **main orchestrator** owns the app-level sidecar lock and injects it into VideoService via `set_sidecar_app_lock`, so only one sidecar batch (TEST or lifecycle) runs at a time. Also `generate_gif_from_clip`. YOLO inference size is configurable via **detection_imgsz** (multi_cam, default 640); higher values improve small-object detection at the cost of speed. At startup, **log_gpu_status** runs nvidia-smi (decode diagnostics); **ensure_detection_model_ready** checks whether the detection model is downloaded. **Docker**: Build from repo root: `docker build -t frigate-buffer:latest .`. FFmpeg from multi-stage build provides NVDEC (decode). Run with NVIDIA Container Toolkit and `NVIDIA_DRIVER_CAPABILITIES=compute,video,utility`. **Install and run:** [INSTALL.md](INSTALL.md).
- **Web / HA**: Flask uses `EventQueryService` to read event/timeline data from disk; **resolve_clip_in_folder** returns the clip basename (dynamic names) for `hosted_clip`/`hosted_clips` URLs. **Event test**: Used **only for the TEST button** (UI). event_test_orchestrator copies event to `events/testN`, deletes sidecars, calls VideoService for sidecars and the same multi-clip extractor used by production; it does not implement any extraction or switching logic. Production multi-cam logic lives in ai_analyzer and multi_clip_extractor. Test folders are cleaned up by retention. HA examples in `examples/home-assistant/`.
...
| `src/frigate_buffer/managers/file.py` | FileManager: storage paths, clip/snapshot download (via DownloadService), export coordination (no transcode), cleanup, path validation (realpath/commonpath). **Cleanup**: `cleanup_old_events` deletes legacy and consolidated event folders older than retention; `events/` subdirs named `test\d+` are deleted by mtime only; folders whose names end with `-canceled` use the same retention (protected only while base event id is in active_event_ids). Helpers: `write_stitched_frame`, `write_ai_frame_analysis_*`, `create_ai_analysis_zip`; `write_ce_summary`, `write_ce_metadata_json`; **rename_event_folder** (append suffix e.g. `-canceled`); **write_canceled_summary** (summary.txt for canceled events); `compute_storage_stats` uses `resolve_clip_in_folder` for clip size. |
| `src/frigate_buffer/services/query.py` | EventQueryService: reads event data from filesystem with TTL and per-folder caching; **resolve_clip_in_folder** for dynamic clip basename; list events (legacy + consolidated), event_by_id, timeline merge. Event dicts include `hosted_clips` / `hosted_clip` with dynamic clip URLs. Used by Flask for event lists and stats. Canceled events (folders ending `-canceled`) appear in event view with title from summary (e.g. "Canceled event: max event length exceeded"). |
| `src/frigate_buffer/config.py` | Load and validate config: voluptuous CONFIG_SCHEMA (cameras, network, settings, ha, gemini, multi_cam, gemini_proxy); merge YAML + env + defaults; flat keys for app (e.g. MQTT_BROKER, GEMINI_PROXY_URL, GEMINI_FRAMES_PER_HOUR_CAP, **MAX_EVENT_LENGTH_SECONDS** default 120). Invalid config exits with code 1. |
...
│   │
│   ├── event_test/                 (only for test-button orchestration; deletable—breaks only TEST button and event_test tests)
│   │   ├── __init__.py             (exports run_test_pipeline)
│   │   └── event_test_orchestrator.py  Test-button orchestrator: allocates testN, copies source, delegates to VideoService/ai_analyzer; no YOLO/lock in this module.
│   │
│   ├── managers/
│   │   ├── __init__.py             (13)
...
│   ├── test_max_event_length.py   (~95)  Max event length: config, FileManager rename/canceled summary, lifecycle and orchestrator (API not sent), cleanup -canceled.
│   ├── test_cleanup_test_folders.py       Cleanup test/canceled folders; -canceled protected when base id active, deleted when past retention.
