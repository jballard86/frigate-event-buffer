...
| `src/frigate_buffer/services/query.py` | **EventQueryService:** read event data from filesystem with TTL and per-folder caching; `resolve_clip_in_folder`; list events, timeline merge. For consolidated events, when `{ce_id}_summary.mp4` exists in the CE root, adds a "Summary video" entry to `hosted_clips` and sets `hosted_clip` to that URL so the player selects it by default. Excludes non-camera dirs (`NON_CAMERA_DIRS`: ultralytics, yolo_models, daily_reports, daily_reviews) from `get_cameras()` and `get_all_events()`. | Flask server (events, stats, player). |
...
| `src/frigate_buffer/web/templates/*.html` | Jinja2 templates for player, stats, daily report, timeline, test run. Player: per-event clip dropdown lists camera clips plus "Summary video" when the compilation file exists; selection follows `hosted_clip` (summary URL when available). Player's AI Analysis tile: first line = event title (canned or quick AI) or "future screenshot analysis"; then scene. | Rendered by Flask. |
