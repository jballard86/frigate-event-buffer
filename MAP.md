...
- **Web / HA**: Flask uses `EventQueryService` to read event/timeline data from disk; **resolve_clip_in_folder** returns the clip basename (dynamic names) for `hosted_clip`/`hosted_clips` URLs. **Event test**: TEST button runs the test pipeline: copy event to `events/testN`, delete detection sidecars, call **VideoService.generate_detection_sidecars_for_cameras** (parallel sidecar in core; event_test receives the same VideoService with app lock injected by orchestrator), build AI payload, write `system_prompt.txt` and `ai_request.html`. Event_test only delegates and remains logic-free (no YOLO, no sidecar lock in event_test). Test folders are cleaned up by retention. HA examples in `examples/home-assistant/`.
