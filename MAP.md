...
| `Dockerfile` | Multi-stage build: lean Python base + FFmpeg/NVDEC from donor; donor stage copies ldd-discovered system libs (libcrypto, libexpat, libgomp, libssl, libz; excludes glibc/toolchain) into `/out/lib`; installs libgomp1 in final stage as fallback; no entrypoint; runs `python -m frigate_buffer.main`. In the same RUN as `pip install -r requirements.txt`, uninstalls `opencv-python` (if present) and installs `opencv-python-headless` only so the container has no X11/libxcb dependency—that layer is cached until `requirements.txt` changes, so src-only builds skip the opencv step. | Build from repo root. |
...
| `src/frigate_buffer/services/video_compilation.py` | Video compilation service. Generates a stitched, cropped, 20fps hardware-accelerated summary video for a CE lifecycle. | Used by lifecycle, orchestrator. |
