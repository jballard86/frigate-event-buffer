...
- **Torch / torchvision** are **pinned** in **`requirements-intel.txt`**; the image installs them from the selected torch wheel stack:\n+  - CPU (default): build with `TORCH_STACK=cpu` (uses `TORCH_INDEX_URL_CPU`, default PyTorch CPU).\n+  - XPU: build with `TORCH_STACK=xpu` (uses `TORCH_INDEX_URL_XPU`, default PyTorch XPU at `https://download.pytorch.org/whl/xpu`).\n+  Builder and runtime must use the **same** stack, or `frigate_intel_decode` can ABI-mismatch libtorch at import time.
- **Smoke (optional):** after build, `docker run --rm … frigate-buffer:intel python3 scripts/smoke_intel_gpu_path.py --strict` checks **`import frigate_intel_decode`** and torch. Add **`--vainfo`** for VA-API output; **`--strict-dri`** with **`--vainfo`** fails if **`vainfo`** is missing or errors (use on Arc hosts with **`/dev/dri`** passed through). Add a bind-mounted **H.264/HEVC** test clip to exercise **QSV** decode: `… python3 scripts/smoke_intel_gpu_path.py --strict /path/in/container/clip.mp4`.
