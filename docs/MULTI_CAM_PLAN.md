...
**Superseded for detection:** See [MULTI_CAM_PLAN_V2.md](MULTI_CAM_PLAN_V2.md) for corrected direction: (1) run detection **on the video** (decoded stream), not on pre-extracted frames; (2) use a **small GPU-capable AI model** (e.g. YOLOv8-nano), not OpenCV HOG.
