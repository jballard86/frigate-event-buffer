...
class TestDownloadService(unittest.TestCase):
    def setUp(self):
        ...
        self.download_service = DownloadService(
            self.frigate_url, self.mock_video_service
        )
    ...
    def test_export_success_false_logs_warning(
        self, mock_sleep, mock_fallback, mock_get, mock_post
    ):
        ...
        for record in self.log_capture:
            if (
                ...
                and "Something went wrong" in record.getMessage()
            ):
                found_warning = True
                break
    ...
    def test_download_404_no_retry(self, mock_get):
        ...
        # Call the method
        result = self.download_service.download_and_transcode_clip(
        ...
        for record in self.log_capture:
            if (
                record.levelno == logging.WARNING
                and "No recording available for event evt_missing"
    ...
    def test_download_400_retries(self, mock_sleep, mock_get):
        ...
        mock_response_400.raise_for_status.side_effect = requests.exceptions.HTTPError(
            "400 Client Error", response=mock_response_400
        )
    ...
    def test_export_422_logs_response_text(self, mock_sleep, mock_fallback, mock_post):
        ...
        found = any(
            ...
            and (
                "response.text" in record.getMessage()
                or "detail" in record.getMessage()
                or "Field required" in record.getMessage()
            )
            for record in self.log_capture
        )
    ...
    def test_export_405_logs_response_text(self, mock_sleep, mock_fallback, mock_post):
        ...
        mock_post.return_value.raise_for_status.side_effect = (
            requests.exceptions.HTTPError("405", response=mock_post.return_value)
        )
        mock_fallback.return_value = {"success": False, "clip_path": None}
        self.download_service.export_and_download_clip(
            event_id="evt1",
    ...
    def test_export_500_logs_response_text(self, mock_sleep, mock_fallback, mock_post):
        ...
        mock_post.return_value = MagicMock(
            status_code=500, ok=False, text='{"detail":"Internal Server Error"}'
        )
        mock_post.return_value.raise_for_status.side_effect = (
            requests.exceptions.HTTPError("500", response=mock_post.return_value)
        )
        mock_fallback.return_value = {"success": False, "clip_path": None}
        self.download_service.export_and_download_clip(
            event_id="evt1",
    ...
    def test_export_non200_empty_body_logs_status(
        ...
        mock_post.return_value.raise_for_status.side_effect = (
            requests.exceptions.HTTPError("503", response=mock_post.return_value)
        )
        mock_fallback.return_value = {"success": False, "clip_path": None}
        self.download_service.export_and_download_clip(
            event_id="evt1",
    ...
    def test_export_in_progress_missing_proceeds_backward_compat(
        ...
        list_resp.raise_for_status = MagicMock()
        list_resp.json.return_value = [
            {"id": "exp1", "export_id": "exp1", "export": "legacy.mp4"}
        ]
        dl_resp = MagicMock()
        dl_resp.raise_for_status = MagicMock()
        dl_resp.iter_content = lambda chunk_size: (b"x" for _ in range(1))
    ...
    def test_download_clip_to_temp_404_returns_failure(self, mock_get):
        """download_clip_to_temp returns success False and clip_path None on 404."""
        mock_resp = MagicMock()
