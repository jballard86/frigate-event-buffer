...
class TestDownloadService(unittest.TestCase):
    ...
    @patch('frigate_buffer.services.download.DownloadService._download_clip_events_api_to_path')  # Mock fallback
    ...
    def test_download_404_no_retry(self, mock_get):
        ...
        # Call the method
        result = self.download_service.download_and_transcode_clip("evt_missing", "/tmp", "cam1")
    ...
    def test_download_400_retries(self, mock_sleep, mock_get):
        ...
        result = self.download_service.download_and_transcode_clip("evt_notready", "/tmp", "cam1")
    ...
    @patch('frigate_buffer.services.download.requests.get')
    @patch('frigate_buffer.services.download.requests.post')
    @patch('frigate_buffer.services.download.DownloadService._download_clip_events_api_to_path')
    @patch('time.sleep')
    ...
    def test_download_clip_to_temp_404_returns_failure(self, mock_get):
        ...
        result = self.download_service.download_clip_to_temp("evt_missing", "/tmp", "cam1")
    ...
    @patch('frigate_buffer.services.download.requests.post')
    @patch('time.sleep')
    def test_export_and_download_clip_returns_clip_path_on_success(self, mock_sleep, mock_post, mock_get):
        ...
        try:
            ...
            result = self.download_service.export_and_download_clip(
                event_id="evt1", folder_path=folder, camera="cam1",
                start_time=1000, end_time=1010, export_buffer_before=0, export_buffer_after=0,
            )
