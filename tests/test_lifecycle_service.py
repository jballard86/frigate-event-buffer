...
from frigate_buffer.models import ConsolidatedEvent, EventState
...
class TestEventLifecycleService(unittest.TestCase):
    ...
    def test_handle_event_new_new_event(self):
        ...
        # Assert
        self.state_manager.create_event.assert_called_with(
            event_id, camera, label, start_time
        )
        self.consolidated_manager.get_or_create.assert_called_with(
            event_id, camera, label, start_time
        )
    ...
    def test_process_event_end(self):
        ...
        self.consolidated_manager.schedule_close_timer.assert_called_with(
    ...
    def test_process_event_end_discards_when_under_minimum_duration(self):
        ...
        # Assert: discard path
        self.file_manager.delete_event_folder.assert_called_with(
            "/tmp/storage/cam1/100_evt1"
        )
    ...
    def test_finalize_consolidated_event(self):
        ...
        self.download_service.export_and_download_clip.return_value = {
            "success": True,
            "clip_path": "/tmp/ce1/cam1/cam1-123.mp4",
        }
    ...
    def test_process_event_end_max_length_does_not_call_clip_ready_or_export(self):
        ...
        self.consolidated_manager.get_by_frigate_event.return_value = None
        self.file_manager.rename_event_folder.return_value = (
            "/tmp/storage/cam1/100_evt1-canceled"
        )

        self.service.process_event_end(event)

        self.file_manager.write_canceled_summary.assert_called_once_with(
            "/tmp/storage/cam1/100_evt1"
        )
        self.notifier.publish_notification.assert_called_once()
        call_args = self.notifier.publish_notification.call_args
    ...
    def test_finalize_consolidated_event_max_length_does_not_call_analysis(self):
        ...
        ce = ConsolidatedEvent(
            ce_id, "folder", "/tmp/storage/events/100_ce1", 100.0, 110.0
        )
        ...
        self.state_manager.get_event.side_effect = lambda fid: (
            evt1 if fid == "evt1" else evt2
        )
        self.file_manager.rename_event_folder.return_value = (
            "/tmp/storage/events/100_ce1-canceled"
        )
    ...
    def test_finalize_consolidated_event_multi_cam_uses_download_then_sidecar(self):
        ...
        self.file_manager.ensure_consolidated_camera_folder.side_effect = (
            lambda base, cam: f"{base}/{cam}"
        )
        ...
        self.video_service.generate_detection_sidecars_for_cameras.return_value = [
            ("cam1", True),
            ("cam2", True),
        ]
